...
if ($f_date_from !== '') {
    $where[]                = 'DATE(mf.created_at) >= :date_from';
    $params[':date_from']   = $f_date_from;
}
if ($f_date_to !== '') {
    $where[]                = 'DATE(mf.created_at) <= :date_to';
    $params[':date_to']     = $f_date_to;
}
if ($f_search !== '') {
    $where[]                = '(mf.observer_name LIKE :s1 OR mf.work_area LIKE :s2 OR mf.comments LIKE :s3)';
    $term                   = '%' . $f_search . '%';
    $params[':s1']          = $term;
    $params[':s2']          = $term;
    $params[':s3']          = $term;
...
$cstmt = $pdo->prepare("SELECT COUNT(*) FROM medical_feedback mf {$whereSql}");
...
<title>Medical Feedback — <?= h($settings['company_name']) ?></title>
...
<div class="main-content">
  <div class="topbar">
    <div class="topbar-left">
      ...
      <h1 class="topbar-title">Medical Feedback</h1>
    </div>
      ...
      <form method="GET" action="" id="filterForm">
        <div class="filter-grid">
          ...
          <div class="form-group">
            ...
            <select name="visit_reason" class="form-input form-select auto-submit">
              <option value="">All Reasons</option>
              <?php foreach ($reason_labels as $val => $label): ?>
                <option value="<?= $val ?>" <?= $f_reason === $val ? 'selected' : '' ?>><?= $label ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group">
          ...
          <div class="form-group filter-search">
            ...
            <input type="text" name="search" class="form-input" placeholder="Name, work area, comments..."
                   value="<?= h($f_search) ?>">
          </div>
          <div class="form-group filter-actions">
            ...
            <div class="btn-row">
              ...
              <a href="<?= BASE_URL ?>/dashboard/medical.php" class="btn btn-outline">Reset</a>
            </div>
          </div>
        </div>
      </form>
    ...
    <div class="card">
      ...
      <?php if (empty($rows)): ?>
        <div class="empty-state">
          ...
          <h4>No medical feedback found</h4>
          <p>Medical clinic feedback submitted through the portal will appear here.</p>
        </div>
      <?php else: ?>
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="td-stars">
                <?= $row['overall_rating'] ? star_rating((int)$row['overall_rating']) : '—' ?>
              </td>
              <td><?= $row['is_anonymous'] ? '<em class="text-muted">Anon</em>' : h($row['observer_name'] ?: '—') ?></td>
              <td><?= $row['urgent_review'] ? '<span class="badge badge-complaint">Urgent</span>' : '—' ?></td>
              <td><span class="badge badge-<?= h($row['status']) ?>"><?= ucfirst(h($row['status'])) ?></span></td>
              <td>
                <a href="<?= BASE_URL ?>/dashboard/medical-detail.php?id=<?= $row['id'] ?>"
